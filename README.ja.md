...
| 検索 | なし | あいまい検索 + 正規表現 |
| AI変換 | なし | 4つのクラウドプロバイダー + ローカルモデル、10個の組み込み + カスタム |
...
いいえ。すべてのデータはマシン上のローカルSQLiteデータベースに保存されます。ローカルAIモデルを使用すれば、データがマシンから出ることは一切ありません。クラウドAIプロバイダー（OpenAI、Gemini、Anthropic、DeepSeek）を使用する場合、明示的に変換を選択したテキストのみが、ご自身のAPIキーを使って直接APIに送信されます。
