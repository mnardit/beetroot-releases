...
いいえ。すべてのデータはマシン上のローカルSQLiteデータベースに保存されます。ローカルAIモデルを使用すれば、データがマシンから出ることは一切ありません。クラウドAIプロバイダー（OpenAI、Gemini、Anthropic、DeepSeek）を使用する場合、明示的に変換を選択したテキストまたは画像のみが、ご自身のAPIキーを使って直接APIに送信されます。
...
**AIビジョンはオフラインで使えますか？**
はい。ローカルビジョンモデル（Ollamaのllavaやmoondreamなど）を使用すれば、データがマシンから出ることはありません。
