...
| AI-трансформации | Нет | 4 облачных провайдера + локальные модели, 10 встроенных + свои |
...
Нет. Всё хранится в локальной базе SQLite на вашем компьютере. С локальной AI-моделью ничего вообще не покидает вашу машину. При использовании облачного провайдера (OpenAI, Gemini, Anthropic или DeepSeek) отправляется только тот текст, который вы явно трансформируете — напрямую в их API с вашим собственным ключом.
