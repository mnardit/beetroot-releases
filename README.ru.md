...
Нет. Всё хранится в локальной базе SQLite на вашем компьютере. С локальной AI-моделью ничего вообще не покидает вашу машину. При использовании облачного провайдера (OpenAI, Gemini, Anthropic или DeepSeek) отправляется только тот текст или изображение, которое вы явно трансформируете — напрямую в их API с вашим собственным ключом.
...
Да, с локальной vision-моделью (например, Ollama с llava или moondream). Данные не покидают ваш компьютер.
